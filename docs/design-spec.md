...
- 開発環境: Docker / Docker Compose を利用する
...
- 開発者ごとの差異を減らすため、ローカル開発はDockerコンテナで統一すること
...
### 8.4 開発環境（Docker）
- 開発時は `docker compose up` でVite開発サーバーを起動する
- Node.js と依存パッケージはコンテナ内で管理する
- ソースはbind mountし、保存時にHMRで即時反映する
- `node_modules` は名前付きvolumeで保持し、ホスト汚染を避ける
...
6. Docker経由で開発サーバーが起動し、ブラウザで確認できる
